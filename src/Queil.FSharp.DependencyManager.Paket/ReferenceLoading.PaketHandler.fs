...
module Logic =
    ...
    let paketVersionSortForNugetCacheFolder somethingAndNames =
        somethingAndNames
        |> Seq.map (fun (something, (name: string)) ->
...
module internal PaketHandler =
    ...
    let MakeDependencyManagerCommand scriptType packageManagerTargetFramework projectRootDirArgument =
        let packageManagerTargetFramework =
            tweakTargetFramework packageManagerTargetFramework

        sprintf
            "install --generate-load-scripts --load-script-type %s --load-script-framework %s project-root \"%s\""
            scriptType
            packageManagerTargetFramework
            (System.IO.Path.GetFullPath projectRootDirArgument)
    ...
    let ResolveDependenciesForLanguage
        ...
        let workingDir =
            Path.Combine(Path.GetTempPath(), "script-packages", string scriptDirHash)
                ...
                if fi.Exists then
                    ...
                    let lines =
                        if List.isEmpty packageManagerTextLinesFromScript then
                            ...
                            (Array.toList depsFileLines)
                            @ ("group Main" :: packageManagerTextLinesFromScript)
        ...
        let loadScript =
            GetPaketLoadScriptLocation
                workingDir
                (Some(tweakTargetFramework targetFramework))
                ("main.group." + fileType)

        let additionalIncludeFolders () =
            [ Path.Combine(workingDir, "paket-files") ] |> List.filter Directory.Exists

        if
            workingDirSpecFile.Exists
        ...
        then
            ...
            let toolPath =
                ...
                | None ->
                    ...
                    let nugetDirs =
                        let nugetDir =
                            DirectoryInfo(Path.Combine(userProfile, ".nuget", "packages", "paket"))
                    ...
                    | None ->
                        ...
                        failwithf
                            "Paket was not found in '%s' or a parent directory, or in those folders:\n\n%s\n\nPlease download the tool and place it in one of the locations."
                            scriptDir
                            foldersTried
            ...
            if p.ExitCode <> 0 then
                ...
                failwithf
                    "Package resolution using '%s' failed, see directory '%s'.%s%s"
                    toolPath
                    workingDir
                    Environment.NewLine
                    msg
    ...
    let ResolveDependencies
        (targetFramework: string, scriptDir: string, scriptName: string, packageManagerTextLinesFromScript: string seq)
        =
        let extension =
