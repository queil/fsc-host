      ...
      let rec handle f (p:Type) s =
        ...
        | f, p when p.IsGenericMethodParameter ->
          if s.GenericParams.ContainsKey(p) && s.GenericParams.[p] <> f then
            failwithf "Failed to add type %s as a substitute for generic parameter type %s. It already has a substitute of type %s" 
...
module Member =
  ...
  let get<'a> (memberPath:string) (assembly:Assembly) =
    ...
    | [Property memberName pi] ->
      pi.GetValue(null) |> tryCast (pi.PropertyType.ToString())
    | [Method memberName mi] ->
      mi |> Reflection.toExpr typeof<'a>
          //|> (fun x -> printfn "%A" x; x)
